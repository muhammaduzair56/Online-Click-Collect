  ...
  return (
    <div className="lg:hidden">
      <button
        ...
        className={`fixed right-4 z-[75] flex h-12 w-12 items-center justify-center rounded-full border border-[#d8c7b8] bg-[#fffaf3]/95 text-[#231f20] shadow-lg backdrop-blur transition hover:bg-white focus:outline-none focus:ring-2 focus:ring-[#c95b63] ${homeScrolled ? "top-3" : "top-[4.45rem]"}`}
        aria-expanded={open}
      ...
      {open && (
        <>
          ...
          <nav
            ...
            className={`fixed right-4 z-[70] w-[min(21rem,calc(100vw-2rem))] rounded-2xl border border-[#d8c7b8] bg-[#fffaf3] p-4 shadow-2xl ${homeScrolled ? "top-[4.75rem]" : "top-[8.2rem]"}`}
          ...
          >
            <div className="grid gap-1 text-sm font-bold text-[#4f413b]">
              ...
              <a onClick={close} className="mt-2 inline-flex items-center gap-2 rounded-xl bg-[#f7dfd8] px-4 py-3 text-[#a8434d]" href={authenticated ? "/profile" : "/login"}>
                {authenticated ? <ShoppingBag size={16} /> : <LogIn size={16} />}
                {authenticated ? "Profile" : "Login"}
              </a>
              {authenticated ? (
                <button type="button" onClick={logout} className="inline-flex items-center gap-2 rounded-xl px-4 py-3 text-left text-[#a8434d] hover:bg-[#f7dfd8]"><LogOut size={16} /> Logout</button>
              ) : (
                <a onClick={close} className="inline-flex items-center gap-2 rounded-xl px-4 py-3 text-[#a8434d] hover:bg-[#f7dfd8]" href="/signup"><UserPlus size={16} /> Create account</a>
              )}
            </div>
          </nav>
        </>
