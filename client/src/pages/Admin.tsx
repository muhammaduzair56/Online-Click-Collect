...
import { Box, CheckCircle2, ChevronDown, ImagePlus, LayoutDashboard, Package, Plus, RefreshCw, Settings2, ShoppingBag, Star, Trash2, Truck, Users } from "lucide-react";
...
export default function Admin() {
  ...
  const uploadImage = async (file?: File) => { if (!file || !galleryProductId) return; setUploading(true); try { const image = await fastApi.uploadProductImage(galleryProductId, file); setGallery((current) => [...current, image]); toast.success("Gallery image uploaded"); } catch (error) { toast.error(error instanceof Error ? error.message : "Could not upload image"); } finally { setUploading(false); } };
  const removeImage = async (id: string) => { try { await fastApi.deleteProductImage(galleryProductId, id); setGallery((current) => current.filter((image) => image.id !== id)); toast.success("Gallery image removed"); } catch (error) { toast.error(error instanceof Error ? error.message : "Could not remove image"); } };
